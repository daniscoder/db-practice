...
-- ------------------------------------------------------------
-- ЗАПРОС 1. Список партнёров с числом доставок,
--           отсортированный по названию.
--           LEFT JOIN — чтобы партнёры без единой отгрузки
...
-- Партнёр и его первая отгрузка одним выражением:
-- CTE с INSERT ... RETURNING отдаёт partner_id сразу во вставку
...
-- отдаёт partner_id в любом случае и заодно обновляет карточку —
...
WHERE pr.product_name = 'Стиральный порошок "Альфа"'
  -- «Первая» отгрузка — значит только если у партнёра ещё нет ни одной.
...
-- Обновление данных существующего партнёра в той же транзакции
UPDATE partners
...
-- и база остаётся в исходном состоянии.


-- ------------------------------------------------------------
-- ЗАПРОС 3. Детальная история отгрузок одного партнёра
--           за указанный период.
...
-- Итоговая строка по тому же партнёру и периоду
SELECT
    p.company_name,
