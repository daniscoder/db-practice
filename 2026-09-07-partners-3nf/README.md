...
Выдача: три партнёра по алфавиту, счётчики 1, 2 и 1, выручка `18000.50`,
...
не заполнен рейтинг и остался `NULL` — не нулём, потому что ноль означал бы
худшую оценку, а партнёр просто не оценён.

Снимок сделан до запроса 2: у «Логистик-Экспресс» ещё исходные `+79991112233`
и `4.80`, которые транзакция потом перепишет. Разницы между `LEFT` и `INNER JOIN`
на этих данных не видно — отгрузки есть у всех троих, партнёра с нулём в
выборке нет.

![Партнёры с числом доставок](task-4-queries/screenshots/partners-with-counts.png)
